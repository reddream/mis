...
表格标签：用于显示列表数据。
	...
	width：表格的宽度。默认100%。
-->
<#macro table value listAction="list.do" class="pn-ltable" style="" theadClass="pn-lthead" 
tbodyClass="pn-ltbody" width="100%" params="" >
...
<#if row_index==0><tbody  class="${tbodyClass}">
<tr onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'">
<#else>
<tr onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'"></#if>
...
function preParams(v){
		var str = $("#parameters").val();
		var arr = str.split("&");
		var s="";
		for(var i=0;i<arr.length;i++){	
		  var vvr = arr[i].split("="); 
          s+="<input type=\"hidden\" name=\""+vvr[0]+"\" value=\""+vvr[1]+"\" />" 
		}
		if(v==undefined)
			$("#hidFields").html(s);
		else{
